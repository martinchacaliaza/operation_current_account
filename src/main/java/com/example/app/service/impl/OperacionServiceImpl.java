...
public class OperacionServiceImpl implements OperacionService {
	...
	public Mono<dtoPerfilConsolidado> perfilConsolidado(String dni) {
		...
		Mono<dtoClient> oper1 = WebClient.builder().baseUrl("http://" + valorget + "/clientes/api/Clientes/").build()
				.get().uri("/dni/" + dni).retrieve().bodyToMono(dtoClient.class).log();
		return oper1.flatMapMany(c1 -> {
			dto.setDni(c1.getDni());
			dto.setApellidoNombre(c1.getApellidos() + " " + c1.getNombres());

			Flux<dtoBank> oper2 = WebClient.builder().baseUrl("http://" + valorget + "/bancos/api/Bancos/").build()
					.get().retrieve().bodyToFlux(dtoBank.class).log();
			return oper2;
		}).flatMap(p -> {
			Flux<dtoCurrentAccount> oper3 = WebClient.builder()
					.baseUrl("http://" + valorget + "/producto_bancario/api/ProductoBancario/").build().get()
					.uri("/dni_codbanco/" + dni + "/" + p.getCodigo_banco()).retrieve()
					.bodyToFlux(dtoCurrentAccount.class).log();
			return oper3;

		}).map(c -> {

			dto.getDesproductos().add(
					new dtoDescProductos(c.getCodigoBancario(), c.getNumeroCuenta(), String.valueOf(c.getSaldo())));
		...
		return oper1.flatMap(c1 -> {
			...
			return valor.flatMap(p -> {
				...
				Mono<dtoCurrentAccount> oper2 = WebClient.builder()
						.baseUrl("http://" + valorget + "/producto_bancario/api/ProductoBancario/").build().put()
	...
	public Mono<OperationCurrentAccount> saveOperacionPagoCredito(OperationCurrentAccount operacion) {
		...
		Mono<dtoCurrentAccount> ope = WebClient.builder()
				.baseUrl("http://" + valorget + "/productos_creditos/api/ProductoCredito/").build().get()
				.uri("/numero_cuenta/" + operacion.getCuentaDestino() + "/" + operacion.getCodigoBancarioDestino())
				.retrieve().bodyToMono(dtoCurrentAccount.class).log();

		return ope.defaultIfEmpty(new dtoCurrentAccount()).flatMap(val -> {

			if (val.getNumeroCuenta() == null) {
				throw new RequestException("La transaccion no se pudo realizar, verifique las cuentas");
			}
			Mono<dtoCurrentAccount> oper1 = WebClient.builder()
					.baseUrl("http://" + valorget + "/producto_bancario/api/ProductoBancario/")
			...
			return oper1.flatMap(c1 -> {
				if (c1.getTipoProducto().getIdTipo().equalsIgnoreCase("1")) {
					...
					if (c1.getSaldo() == 20) {
						...
						throw new RequestException("Ya no puede realizar retiros, debe tener un monton minimo"
								+ " de S/.20 en su cuenta.");
					}
				}
